...
parser.add_argument('--noise_mode', default='worse_label', help='aggre_label, random_label3, worse_label')
parser.add_argument('--noise_path', default='./dataset/CIFAR-10_human.pt')
...
def train(epoch, net, optimizer, trainloader_v, trainloader_v_tilde):
    ...
    for batch_idx in range(num_iter * args.cls_num):
        ...
        if batch_idx % 100 == 0:
            pprint('%s:%s | Training | Iter[%3d/%3d]\t  Labeled loss: %.2f\t'
                % (args.dataset, args.noise_mode, batch_idx + 1, num_iter * args.cls_num, train_loss.item()), text_name)
...
def warmup(epoch, net, optimizer, dataloader):
    ...
    for batch_idx, (inputs, labels, path) in enumerate(dataloader):
        ...
        if batch_idx % 100 == 0:
            pprint('%s:%s | WarmUp | Iter[%3d/%3d]\t CE-loss: %.4f\t' % (args.dataset,
...
text_name = './Log_cifarN/%s(%s)_beta(%.1f)_ClsNum(%.1f)_%s' % (args.dataset, args.noise_mode, args.beta, args.cls_num, args.mixup)
...
# dataset, noise_type, noise_path, is_human, batch_size, num_workers, root_dir, log, noise_file='', noise_mode='cifarn'
loader = dataloader.cifarn_dataloader(args.dataset, noise_type=args.noise_mode, noise_path=args.noise_path, batch_size=args.batch_size,
